...
pie(feeds[order(feeds, decreasing = TRUE)],
    ...
    clockwise = TRUE, # Defaults is FALSE)
...
# Changing graphical paramters for a minute
...
# Note that cin,cra,csi,cxy and din are read-only parameters
# that were written to oldpar but cannot be rewritten; just ignore the warning messages for these.
